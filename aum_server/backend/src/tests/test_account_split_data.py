...
from src.account_split_data import AccountsSplitData
...
class TestAccountsSplitData:
    ...
    def test_get_random_accounts(self, mocker):
        mocker_accounts = mocker.patch(
            'src.account_split_data.AccountsSplitData.accounts',
        )
        mocker_choices = mocker.patch(
            'src.account_split_data.random.choices', return_value=True
        )
    ...
    def test_get_random_splits_percents(self, mocker):
        mocker_ones = mocker.patch('src.account_split_data.numpy.ones', return_value=1)
        mocker_multinomial = mocker.patch(
            'src.account_split_data.numpy.random.multinomial', return_value=[1]
        )
        assert AccountsSplitData()._get_random_splits_percents(1) == 1
        mocker_ones.assert_has_calls([call(1), call(1), call(1)])
        mocker_multinomial.assert_called_once_with(n=100, pvals=1.0, size=1)

    def test_get_random_account_split_data(self, mocker):
        mocker.patch(
            'src.account_split_data.AccountsSplitData._get_random_accounts',
            return_value=['a', 'b'],
        )
        mocker.patch(
            'src.account_split_data.AccountsSplitData._get_random_splits_percents',
            return_value=[1, 2],
        )
