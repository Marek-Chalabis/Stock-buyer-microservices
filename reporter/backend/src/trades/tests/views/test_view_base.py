...
from src.trades.forms import BuyTradesForm
...
class TestBaseSellBuyTradeView:
    ...
    def test_dispatch_request_form_handlers(self, mocker, class_obj):
        mocker.patch('src.trades.view_base.render_template')
        mocker_form_handlers = mocker.patch(
            'src.trades.view_base.BaseSellBuyTradeView._form_handlers',
        )
    ...
    def test_dispatch_request_render_template(self, mocker, class_obj):
        mocker_render_template = mocker.patch('src.trades.view_base.render_template')
        mocker.patch('src.trades.view_base.BaseSellBuyTradeView._form_handlers')
    ...
    def test_form_handlers(
        ...
        with app.test_request_context():
            mocker_request = mocker.patch('src.trades.view_base.request')
            mocker_request.form = tested_form
            mocker_expected_method = mocker.patch(
                f'src.trades.view_base.BaseSellBuyTradeView.{expected_method_call}',
            )
    ...
    def test_handle_buy_trades_form_invalid_form(
        ...
        mocker_flash_errors_from_form = mocker.patch(
            'src.trades.view_base.flash_errors_from_form',
        )
    ...
    def test_handle_buy_trades_form_missing_money(
        ...
        mocker_flash = mocker.patch('src.trades.view_base.flash')
        mocker.patch('src.trades.view_base.request.form.get', return_value='TEST')
